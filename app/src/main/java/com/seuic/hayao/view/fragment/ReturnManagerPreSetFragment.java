...
import com.seuic.hayao.presenter.impl.ReturnManagerPreSetPresenterImpl;
import com.seuic.hayao.view.ReturnManagerPreSetView;
import com.seuic.hayao.view.impl.ReturnManagerActivity;
...
public class ReturnManagerPreSetFragment extends Fragment implements View.OnClickListener, ReturnManagerPreSetView {
    ...
    private ReturnManagerPresenter mPresenter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View view = LayoutInflater.from(getActivity()).inflate(R.layout.fragment_return_manager_pre_set, null);
...
//            Toast.makeText(getActivity(),"请稍等..",Toast.LENGTH_SHORT).show();
    ...
    public void updateBillType(ArrayList<StoreTypeInfo> types) {
        ...
        for (StoreTypeInfo info : types) {
            if ("退货入库".equals(info.getStoreTypeText())) {
                infos.add(0, info);
            }
            if ("退货出库".equals(info.getStoreTypeText())) {
                infos.add(info);
            }
        }
    ...
    public void onCheckFinished(Boolean isExit,String billNumber) {
        if (!isExit) {
            ((ReturnManagerActivity) getActivity()).forwardToNext(billNumber, (StoreTypeInfo) mSpAdapter.getItem(mBillTypeSp.getSelectedItemPosition()));
        } else {
